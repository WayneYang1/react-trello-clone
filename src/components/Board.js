...
class Board extends Component {
  ...
  handleDrop = ({ source, destination, type }) => {
    // if the draggable is dropped outside the allowed zones
    if (!destination) return;

    const { dispatch } = this.props;

    // Moves column or list
    if (type === "COLUMN") {
      // Prevents any updates if nothing has changed
